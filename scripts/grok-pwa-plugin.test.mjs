...
/** Isolate head injection from this app's site.json / public/og.jpg (title Quire, custom card). */
const EMPTY_CWD = mkdtempSync(join(tmpdir(), "grok-pwa-empty-"));

function inject(html, ctx = {}) {
  return injectGrokPwaHead(html, {
    ...ctx,
    cwd: ctx.cwd ?? EMPTY_CWD,
    site: ctx.site !== undefined ? ctx.site : {},
  });
}

function headInjector(ctx = {}) {
  return createHeadInjector({
    ...ctx,
    cwd: ctx.cwd ?? EMPTY_CWD,
    site: ctx.site !== undefined ? ctx.site : {},
  });
...
test("published VITE_PUBLIC_HOSTNAME wins over request Host for og:image", () => {
  ...
  try {
    const vercelHost = inject("<html><head><title>RACK</title></head></html>", {
    ...
    const otherPublicHost = inject("<html><head><title>RACK</title></head></html>", {
...
test("vercel Host without a public hostname emits no og:image", () => {
  ...
  try {
    const out = inject("<html><head><title>RACK</title></head></html>", {
      host: "01a020b6-803a-71a2-bb47-e2bec57eb9a2-662k8x1l1-xai-org.vercel.app",
      site: { title: "RACK", card: "custom" },
    });
    assert.doesNotMatch(out, /property="og:image"/);
...
test("streaming injector matches </HEAD> case-insensitively", () => {
  const injector = headInjector({ appName: "Wild Race" });
  const chunks = [
...
test("vite config keeps the nitro serverDir wiring", () => {
  const viteConfig = readFileSync(join(TEMPLATE_ROOT, "vite.config.ts"), "utf8").replace(/\r\n/g, "\n");
  // Quire may use a ternary (android → false); still require "./server" on the desktop/web path.
  assert.match(viteConfig, /serverDir:[\s\S]*?"\.\/server"/);
